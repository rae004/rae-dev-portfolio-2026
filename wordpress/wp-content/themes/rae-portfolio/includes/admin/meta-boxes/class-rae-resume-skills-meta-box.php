	...
	private function render_skills_interface( array $skills_by_category, array $selected_skills ): void {
		...
		<div class="rae-resume-skills-selector">
			<div class="skills-search-container" style="margin-bottom: 20px;">
				...
				<p class="description">
					Search and select skills related to this resume item. Selected skills will appear highlighted.
				</p>
			</div>
							...
							if ( $skill_post ) :
								...
								>
									...
									<span class="remove-skill"
											style="cursor: pointer; margin-left: 5px; font-weight: bold;">&times;</span>
								</span>
							<?php endif; ?>
						<?php endforeach; ?>
					<?php endif; ?>
	...
	private function add_scripts_and_styles() {
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				...
				const $noSkillHtml =
					'<span class="no-skills-selected" style="color: #5e5e5e; font-style: italic;">No skills selected</span>';
				...
				function getSkillHtml(skill) {
					...
					const pillStyles = pillStylesArray.join(' ');
					const parentElement = document.createElement('span');
					parentElement.setAttribute('class', 'selected-skill-pill');
					parentElement.setAttribute('data-skill-id', skill.id);
					parentElement.setAttribute('style', pillStyles);

					const childElement = document.createElement('span');
					childElement.setAttribute('class', 'remove-skill');
					childElement.setAttribute('style', 'cursor: pointer; margin-left: 5px; font-weight: bold;');
					childElement.innerHTML = '&times;';

					parentElement.innerHTML = skill.name + ' ';
				...
				function updateSelectedSkillsDisplay() {
					...
					if (selectedSkills.length === 0) {
						$selectedDisplay.html($noSkillHtml);
					} else {
						let html = '';
						selectedSkills.forEach(function(skill) {
							html += getSkillHtml(skill);
						});
	...
	public function save_meta_data( int $post_id ): void {
		...
		if ( ! isset( $_POST['rae_resume_skills_nonce_field'] ) ||
			! wp_verify_nonce(
				sanitize_text_field(
					wp_unslash(
						$_POST['rae_resume_skills_nonce_field']
					)
				),
				'rae_resume_skills_nonce'
			)
		) {
			return;
		}
